...
start_sg = time.time()
# Génération de 100 échantillons bruités (nt_samples) autour de l'image
attributions_sg = noise_tunnel.attribute(
    input_tensor, 
    nt_samples=100, 
    nt_type='smoothgrad', 
    target=predicted_class_idx,
    stdevs=0.1,               # Écart-type du bruit adapté à la normalisation
    internal_batch_size=2     # Protège la VRAM
)
end_sg = time.time()
