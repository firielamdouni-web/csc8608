...
def redact_order_id(text: str) -> Tuple[str, int]:
    """
    Masque un identifiant après 'order number is' OU 'other number is'
    même s'il est épelé (a.x.1.9.7.3.5).
    """
    count = 0
    # On ajoute "other" en alternative pour couvrir les erreurs ASR
    pattern = re.compile(
        r"\b(?:order|other) number is\b\s+([a-z0-9\s\.\-]{3,80})",
        re.IGNORECASE
    )
    def _sub(m):
        nonlocal count
